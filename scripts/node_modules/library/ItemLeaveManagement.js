/* 
		WARNING 
		Auto generated file. 
		Do not modify its contents.
*/

const extend = require('js-base/core/extend');
const FlexLayout = require('sf-core/ui/flexlayout');
const Color = require('sf-core/ui/color');
const ImageView = require('sf-core/ui/imageview');
const ImageFillType = require('sf-core/ui/imagefilltype');
const Image = require('sf-core/ui/image');
const Label = require('sf-core/ui/label');
const TextAlignment = require('sf-core/ui/textalignment');
const Font = require('sf-core/ui/font');

const HorizontalDivider = require("../../components/HorizontalDivider");
const VerticalDivider = require("../../components/VerticalDivider");
const ListItem1 = require("../../components/ListItem1");

const getCombinedStyle = require("library/styler-builder").getCombinedStyle;

const ItemLeaveManagement = extend(FlexLayout)(
	//constructor
	function(_super, props) {
		// initalizes super class for this component scope
		_super(this, props);

		const dataContainerStyle = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 103,
			flexGrow: 1,
			flexDirection: FlexLayout.FlexDirection.ROW
		});
		var dataContainer = new FlexLayout(dataContainerStyle);
		this.addChild(dataContainer);
		
		const dividerStyle = getCombinedStyle(".flexLayout", {
			left: 0,
			top: 0,
			width: null,
			height: 1,
			backgroundColor: Color.create(255, 255, 255, 255),
			alpha: 1,
			id: 102,
			flexGrow: null,
			positionType: FlexLayout.PositionType.RELATIVE
		});
		var divider = new HorizontalDivider(dividerStyle, "itemLeaveManagement");
		this.addChild(divider);
		
		const approveContainerStyle = getCombinedStyle(".flexLayout", {
			height: 50,
			width: null,
			backgroundColor: Color.create(255, 255, 255, 255),
			paddingLeft: 10,
			paddingRight: 10,
			id: 101,
			flexDirection: FlexLayout.FlexDirection.ROW
		});
		var approveContainer = new FlexLayout(approveContainerStyle);
		this.addChild(approveContainer);
		
		const flexlayout1Style = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 1032,
			flexGrow: 1
		});
		var flexlayout1 = new FlexLayout(flexlayout1Style);
		dataContainer.addChild(flexlayout1);
		
		const cont1Style = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 1015,
			alignItems: FlexLayout.AlignItems.CENTER,
			justifyContent: FlexLayout.JustifyContent.CENTER,
			flexGrow: 1,
			flexDirection: FlexLayout.FlexDirection.ROW
		});
		var cont1 = new FlexLayout(cont1Style);
		approveContainer.addChild(cont1);
		
		const flexlayout10Style = getCombinedStyle(".flexLayout", {
			backgroundColor: Color.create(255, 151, 151, 151),
			alpha: 0.4,
			left: 0,
			top: 0,
			width: 1,
			height: null,
			marginTop: 10,
			marginBottom: 10,
			id: 1014,
			positionType: FlexLayout.PositionType.RELATIVE
		});
		var flexlayout10 = new VerticalDivider(flexlayout10Style, "itemLeaveManagement");
		approveContainer.addChild(flexlayout10);
		
		const flexlayout2Style = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 1031,
			flexGrow: 2
		});
		var flexlayout2 = new FlexLayout(flexlayout2Style);
		dataContainer.addChild(flexlayout2);
		
		const cont2Style = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 1013,
			alignItems: FlexLayout.AlignItems.CENTER,
			justifyContent: FlexLayout.JustifyContent.CENTER,
			flexGrow: 1,
			flexDirection: FlexLayout.FlexDirection.ROW
		});
		var cont2 = new FlexLayout(cont2Style);
		approveContainer.addChild(cont2);
		
		const flexlayout10_1Style = getCombinedStyle(".flexLayout", {
			backgroundColor: Color.create(255, 151, 151, 151),
			alpha: 0.4,
			left: 0,
			top: 0,
			width: 1,
			height: null,
			marginTop: 10,
			marginBottom: 10,
			id: 1012,
			positionType: FlexLayout.PositionType.RELATIVE
		});
		var flexlayout10_1 = new VerticalDivider(flexlayout10_1Style, "itemLeaveManagement");
		approveContainer.addChild(flexlayout10_1);
		
		const cont3Style = getCombinedStyle(".flexLayout", {
			height: null,
			width: null,
			id: 1011,
			alignItems: FlexLayout.AlignItems.CENTER,
			justifyContent: FlexLayout.JustifyContent.CENTER,
			flexGrow: 1,
			flexDirection: FlexLayout.FlexDirection.ROW
		});
		var cont3 = new FlexLayout(cont3Style);
		approveContainer.addChild(cont3);
		
		const approveIconStyle = getCombinedStyle(".imageView", {
			height: 20,
			width: 20,
			imageFillType: ImageFillType.ASPECTFIT,
			image: Image.createFromFile("images://icon_approve_active.png"),
			id: 10152
		});
		var approveIcon = new ImageView(approveIconStyle);
		cont1.addChild(approveIcon);
		this.approveIcon = approveIcon;

		const deleteIconStyle = getCombinedStyle(".imageView", {
			height: 20,
			width: 20,
			imageFillType: ImageFillType.ASPECTFIT,
			image: Image.createFromFile("images://icon_delete.png"),
			id: 10112
		});
		var deleteIcon = new ImageView(deleteIconStyle);
		cont3.addChild(deleteIcon);
		this.deleteIcon = deleteIcon;

		const daysCountStyle = getCombinedStyle(".label .label-list-item-header", {
			height: null,
			width: null,
			textAlignment: TextAlignment.BOTTOMCENTER,
			text: "3",
			id: 10322,
			flexGrow: 1,
			font: Font.create("Arial", 22, Font.BOLD)
		});
		var daysCount = new Label(daysCountStyle);
		if(daysCountStyle.scrollEnabled === false)
			daysCount.ios && (daysCount.ios.scrollEnabled = false);
		flexlayout1.addChild(daysCount);
		this.daysCount = daysCount;

		const editIconStyle = getCombinedStyle(".imageView", {
			height: 20,
			width: 20,
			imageFillType: ImageFillType.ASPECTFIT,
			image: Image.createFromFile("images://icon_edit_active.png"),
			id: 10132
		});
		var editIcon = new ImageView(editIconStyle);
		cont2.addChild(editIcon);
		this.editIcon = editIcon;

		const startDateStyle = getCombinedStyle(".flexLayout", {
			left: 0,
			top: 0,
			width: 220,
			height: null,
			backgroundColor: Color.create(255, 255, 255, 255),
			id: 10312,
			flexDirection: FlexLayout.FlexDirection.ROW,
			positionType: FlexLayout.PositionType.RELATIVE,
			flexGrow: 1
		});
		var startDate = new ListItem1(startDateStyle, "itemLeaveManagement");
		flexlayout2.addChild(startDate);
		this.startDate = startDate;

		const label205_1Style = getCombinedStyle(".label .label-list-item-header", {
			height: null,
			width: null,
			textAlignment: TextAlignment.MIDCENTER,
			text: "Days",
			id: 10321,
			flexGrow: 1,
			font: Font.create("Arial", 18, Font.NORMAL)
		});
		var label205_1 = new Label(label205_1Style);
		if(label205_1Style.scrollEnabled === false)
			label205_1.ios && (label205_1.ios.scrollEnabled = false);
		flexlayout1.addChild(label205_1);
		
		const approveLabelStyle = getCombinedStyle(".label .label-approve-item-active", {
			height: 30,
			width: null,
			marginLeft: 5,
			minWidth: 50,
			text: "Approve",
			maxWidth: 100,
			id: 10151,
			flexGrow: null
		});
		var approveLabel = new Label(approveLabelStyle);
		if(approveLabelStyle.scrollEnabled === false)
			approveLabel.ios && (approveLabel.ios.scrollEnabled = false);
		cont1.addChild(approveLabel);
		this.approveLabel = approveLabel;

		const editLabelStyle = getCombinedStyle(".label .label-edit-item-active", {
			height: 30,
			width: null,
			marginLeft: 5,
			minWidth: 30,
			text: "Edit",
			id: 10131
		});
		var editLabel = new Label(editLabelStyle);
		if(editLabelStyle.scrollEnabled === false)
			editLabel.ios && (editLabel.ios.scrollEnabled = false);
		cont2.addChild(editLabel);
		this.editLabel = editLabel;

		const deleteLabelStyle = getCombinedStyle(".label .label-delete-item-active", {
			height: 30,
			width: null,
			marginLeft: 5,
			minWidth: 40,
			text: "Delete",
			id: 10111
		});
		var deleteLabel = new Label(deleteLabelStyle);
		if(deleteLabelStyle.scrollEnabled === false)
			deleteLabel.ios && (deleteLabel.ios.scrollEnabled = false);
		cont3.addChild(deleteLabel);
		this.deleteLabel = deleteLabel;

		const endDateStyle = getCombinedStyle(".flexLayout", {
			left: 0,
			top: 0,
			width: 220,
			height: null,
			backgroundColor: Color.create(255, 255, 255, 255),
			id: 10311,
			flexDirection: FlexLayout.FlexDirection.ROW,
			positionType: FlexLayout.PositionType.RELATIVE,
			flexGrow: 1
		});
		var endDate = new ListItem1(endDateStyle, "itemLeaveManagement");
		flexlayout2.addChild(endDate);
		this.endDate = endDate;

		//assign the children to itemLeaveManagement 
		this.children = Object.assign({}, {
			dataContainer: dataContainer,
			divider: divider,
			approveContainer: approveContainer
		});
		
		//assign the children of dataContainer
		dataContainer.children = Object.assign({}, {
			flexlayout1: flexlayout1,
			flexlayout2: flexlayout2
		});
		
		//assign the children of approveContainer
		approveContainer.children = Object.assign({}, {
			cont1: cont1,
			flexlayout10: flexlayout10,
			cont2: cont2,
			flexlayout10_1: flexlayout10_1,
			cont3: cont3
		});
		
		//assign the children of flexlayout1
		flexlayout1.children = Object.assign({}, {
			daysCount: daysCount,
			label205_1: label205_1
		});
		
		//assign the children of cont1
		cont1.children = Object.assign({}, {
			approveIcon: approveIcon,
			approveLabel: approveLabel
		});
		
		//assign the children of flexlayout2
		flexlayout2.children = Object.assign({}, {
			startDate: startDate,
			endDate: endDate
		});
		
		//assign the children of cont2
		cont2.children = Object.assign({}, {
			editIcon: editIcon,
			editLabel: editLabel
		});
		
		//assign the children of cont3
		cont3.children = Object.assign({}, {
			deleteIcon: deleteIcon,
			deleteLabel: deleteLabel
		});
		
	});

ItemLeaveManagement.defaults = getCombinedStyle(".flexLayout", {
	width: 350,
	height: 124,
	backgroundColor: Color.create(0, 255, 255, 255),
	paddingBottom: 10,
	flexGrow: 1,
	positionType: FlexLayout.PositionType.RELATIVE
});

module && (module.exports = ItemLeaveManagement);