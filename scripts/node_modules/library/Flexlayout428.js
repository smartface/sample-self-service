/* 
		WARNING 
		Auto generated file. 
		Do not modify its contents.
*/

const extend = require('js-base/core/extend');
const FlexLayout = require('sf-core/ui/flexlayout');
const Label = require('sf-core/ui/label');
const Color = require('sf-core/ui/color');
const ImageView = require('sf-core/ui/imageview');
const Image = require('sf-core/ui/image');
const ImageFillType = require('sf-core/ui/imagefilltype');

const HorizontalDivider = require("../../components/HorizontalDivider");

const getCombinedStyle = require("library/styler-builder").getCombinedStyle;

const Flexlayout428 = extend(FlexLayout)(
	//constructor
	function(_super, props) {
		// initalizes super class for this component scope
		_super(this, props);

		const flexlayout429Style = getCombinedStyle(".flexLayout", {
			height: 30,
			width: null,
			paddingLeft: 10,
			flexGrow: null,
			flexDirection: FlexLayout.FlexDirection.ROW,
			alignItems: FlexLayout.AlignItems.CENTER
		});
		var flexlayout429 = new FlexLayout(flexlayout429Style);
		this.addChild(flexlayout429);
		
		const label1402Style = getCombinedStyle(".label .label-list-item-value", {
			width: null,
			height: 32,
			marginLeft: 10,
			marginRight: 10,
			text: "",
			flexGrow: null
		});
		var label1402 = new Label(label1402Style);
		this.addChild(label1402);
		
		const flexlayout430Style = getCombinedStyle(".flexLayout", {
			left: 0,
			top: 0,
			width: null,
			height: 1,
			backgroundColor: Color.create(255, 255, 255, 255),
			alpha: 1,
			flexGrow: null,
			positionType: FlexLayout.PositionType.RELATIVE
		});
		var flexlayout430 = new HorizontalDivider(flexlayout430Style, "flexlayout428");
		this.addChild(flexlayout430);
		
		const label1403Style = getCombinedStyle(".label .label-list-item-title", {
			width: 80,
			height: 30,
			text: "Text"
		});
		var label1403 = new Label(label1403Style);
		flexlayout429.addChild(label1403);
		
		const imageview2Style = getCombinedStyle(".imageView", {
			width: 10,
			height: 5,
			image: Image.createFromFile("images://arrow_down.png"),
			imageFillType: ImageFillType.ASPECTFIT
		});
		var imageview2 = new ImageView(imageview2Style);
		flexlayout429.addChild(imageview2);
		
		//assign the children to flexlayout428 
		this.children = Object.assign({}, {
			flexlayout429: flexlayout429,
			label1402: label1402,
			flexlayout430: flexlayout430
		});
		
		//assign the children of flexlayout429
		flexlayout429.children = Object.assign({}, {
			label1403: label1403,
			imageview2: imageview2
		});
		
	});

Flexlayout428.defaults = getCombinedStyle(".flexLayout", {
	width: 207,
	height: 66
});

module && (module.exports = Flexlayout428);